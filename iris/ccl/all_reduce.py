...
VARIANT_PUSH = "push"
VARIANT_TWO_SHOT_PUSH = "two_shot_push"
VARIANT_AUTO = "auto"

# Byte threshold (per the full-buffer size M*N*elem) below which the read-based
# two_shot beats the write-based two_shot_push. Measured crossover on 8x MI300X
# two_shot wins at <=2MB (1024^2 bf16 -> 1.07x vs RCCL), two_shot_push
# wins from ~8MB up. 4MB sits in the middle of the crossover.
_AUTO_PUSH_BYTES = 4 << 20
...
def all_reduce_preamble(
    ...
    if variant not in _ALL_VARIANTS:
        raise ValueError(f"Invalid all_reduce_variant: {variant}. Must be one of: {_ALL_VARIANTS}")
    ...
    elif variant == VARIANT_PUSH:
        # Push (PUT) all-reduce needs a (world_size * M, N) scratch landing
        # zone: rank r scatters its input into rows [r*M:(r+1)*M] of every
        # peer's scratch, then each rank reduces the world_size local slots.
        if (
            workspace.scratch is None
            or workspace.scratch.shape != (world_size * M, N)
            or workspace.scratch.dtype != dtype
        ):
            workspace.scratch = shmem.zeros((world_size * M, N), dtype=dtype)
        else:
            workspace.scratch.zero_()
        shmem.barrier()
    ...
    elif variant in (VARIANT_TWO_SHOT, VARIANT_TWO_SHOT_PUSH):
        if variant == VARIANT_TWO_SHOT_PUSH:
            if M % world_size != 0:
                raise ValueError(f"two_shot_push requires M ({M}) divisible by world_size ({world_size})")
            Mb = M // world_size
            if Mb % config.block_size_m != 0:
                raise ValueError(
                    f"two_shot_push requires M/world_size ({Mb}) divisible by block_size_m ({config.block_size_m})"
                )
            if N % config.block_size_n != 0:
                raise ValueError(f"two_shot_push requires N ({N}) divisible by block_size_n ({config.block_size_n})")
            chunk_n = getattr(config, "all_reduce_push_chunk_n", 0) or N
            chunk_n = min(chunk_n, N)
            if N % chunk_n != 0:
                raise ValueError(f"two_shot_push requires N ({N}) divisible by all_reduce_push_chunk_n ({chunk_n})")
            if chunk_n % config.block_size_n != 0:
                raise ValueError(
                    f"two_shot_push requires chunk_n ({chunk_n}) divisible by block_size_n ({config.block_size_n})"
                )
            if workspace.scratch is None or workspace.scratch.shape != (M, chunk_n) or workspace.scratch.dtype != dtype:
                workspace.scratch = shmem.zeros((M, chunk_n), dtype=dtype)
                # Only needed once, on (re)allocation, to register the symmetric
                # heap slot across ranks. Steady-state calls skip this host barrier.
                shmem.barrier()
...
@triton.jit()
def persistent_all_reduce_push_scatter(
    input_ptr,
    scratch_ptr,
    M,
    N,
    ...
    """
    Push (PUT) all-reduce — scatter phase.

    Each rank WRITES its local input into rows [group_rank*M:(group_rank+1)*M]
    of every peer's scratch buffer (a remote WRITE that crosses xGMI as a
    store). This is the direction-flipped twin of ``one_shot`` (which READs
    every peer's input). After this kernel + a barrier, each rank's scratch
    holds all world_size partials locally and is reduced by the reduce kernel.

    Bytes crossing xGMI are (world_size-1)*M*N per rank, identical to one_shot's
    remote reads — only the direction differs.
    """
    pid = tl.program_id(0)
    ...
    for tile_id in range(pid, total_tiles, COMM_SMS):
        ...
        # scratch is a contiguous (world_size*M, N) buffer (row stride == N);
        # this rank owns slot group_rank at rows [group_rank*M:(group_rank+1)*M].
        rm_scratch = rm + group_rank * M
        scratch_offset = rm_scratch[:, None] * N + rn[None, :]
        scratch_ptr_local = scratch_ptr + scratch_offset

        # Stagger destination order per rank to spread receiver-side HBM load.
        for j in tl.static_range(world_size):
            i = (group_rank + j) % world_size
            dest_rank = rank_start + i * rank_stride
            if i == group_rank:
                tl.store(scratch_ptr_local, data, mask=mask, cache_modifier=".wt")
            else:
                iris.store(
                    scratch_ptr_local,
                    data,
...
@triton.jit()
def persistent_all_reduce_push_reduce(
    scratch_ptr,
    output_ptr,
    M,
    N,
    ...
    """
    Push (PUT) all-reduce — local reduce phase.

    Every rank's scratch now holds all world_size partials (slot i in rows
    [i*M:(i+1)*M]). Sum the slots locally and write the result to output. This
    phase is entirely local (no xGMI traffic).
    """
    pid = tl.program_id(0)
...
@triton.jit()
def persistent_all_reduce_ts_push_scatter(
    input_ptr,
    scratch_ptr,
    M,
    N,
    ...
    """Two-shot PUSH all-reduce — reduce-scatter scatter phase (all WRITES).

    Row-block partition: owner r owns global rows [r*Mb:(r+1)*Mb], Mb=M/world_size.
    This rank WRITES each of its input row-blocks into the owning peer's scratch,
    landing in slot ``group_rank`` (rows [group_rank*Mb:(group_rank+1)*Mb] of that
    peer's scratch). Remote write volume = (world_size-1)/world_size * M*N — no
    remote reads, so no LFIFO read-return contention.
    """
    pid = tl.program_id(0)
    Mb = M // world_size
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    pids_per_owner = Mb // BLOCK_SIZE_M
    tiles_per_owner = pids_per_owner * num_pid_n

    # Iterate so consecutive program-ids target DIFFERENT owners (owner rotates
    # with c). This keeps all xGMI links busy concurrently instead of every active
    # WG hammering one peer at a time. Only the world_size-1 REMOTE owners are
    # visited — the self block is read straight from ``input`` in the gather phase,
    # so it is never loaded or scattered here.
    remote_tiles = (world_size - 1) * tiles_per_owner

    for c in range(pid, remote_tiles, COMM_SMS):
        owner_slot = c % (world_size - 1)
        tile_in_owner = c // (world_size - 1)
        # Map slot in [0, world_size-1) to an owner != group_rank.
        owner = owner_slot if owner_slot < group_rank else owner_slot + 1
        local_pid_m = tile_in_owner // num_pid_n
        pid_n = tile_in_owner % num_pid_n
        pid_m = owner * pids_per_owner + local_pid_m
        scratch_pid_m = group_rank * pids_per_owner + local_pid_m
        ...
        data = tl.load(input_ptr + rm[:, None] * stride_in_m + rn[None, :] * stride_in_n)

        sm = scratch_pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
        sm = tl.max_contiguous(tl.multiple_of(sm, BLOCK_SIZE_M), BLOCK_SIZE_M)
        scratch_off = sm[:, None] * N + rn[None, :]

        dest_rank = rank_start + owner * rank_stride
        iris.store(scratch_ptr + scratch_off, data, iris_rank, dest_rank, heap_bases, hint=(1, BLOCK_SIZE_N))


@triton.jit()
def persistent_all_reduce_ts_push_gather(
    scratch_ptr,
    output_ptr,
    input_ptr,
    M,
    ...
    """Two-shot PUSH all-reduce — local reduce + all-gather broadcast (all WRITES).

    This rank owns global rows [group_rank*Mb:(group_rank+1)*Mb]. Its scratch now
    holds world_size contributions for that block (slot s = rows [s*Mb:(s+1)*Mb]).
    Sum the slots, write the reduced block to local output, then WRITE it to every
    peer's output at the same global rows. Remote write volume =
    (world_size-1)/world_size * M*N.
    """
    pid = tl.program_id(0)
    Mb = M // world_size
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    ...
    for tile in range(pid, owned_tiles, COMM_SMS):
        ...
        # Own contribution comes straight from input (self slot is never written
        # to scratch); remaining world_size-1 contributions come from scratch.
        ...
        tl.store(output_ptr + out_off, reduced, cache_modifier=".wt")
        for j in tl.static_range(world_size):
            i = (group_rank + j) % world_size
            if i != group_rank:
                dest_rank = rank_start + i * rank_stride
                iris.store(output_ptr + out_off, reduced, iris_rank, dest_rank, heap_bases, hint=(1, BLOCK_SIZE_N))
...
# Cache for internally-managed workspaces (when the caller does not supply one),
# keyed by (id(shmem), variant, M, N, dtype). Without this, every public
# all_reduce() call reallocates symmetric-heap scratch (a fixed multi-ms cost
# plus eventual heap exhaustion under repeated large-size calls).
_AR_WORKSPACE_CACHE: dict = {}
...
def all_reduce(
    ...
    # Size-based variant selection (mirrors RCCL protocol selection): the
    # read-based two_shot has a much lower fixed cost (1 kernel + 1 barrier) and
    # wins in the latency regime; the write-based two_shot_push avoids xGMI
    # read-return contention and wins once bandwidth-bound. two_shot_push also
    # requires M divisible by world_size, so fall back to two_shot if it isn't.
    if variant == VARIANT_AUTO:
        nbytes = M * N * input_tensor.element_size()
        if nbytes >= _AUTO_PUSH_BYTES and (M % world_size == 0):
            variant = VARIANT_TWO_SHOT_PUSH
        else:
            variant = VARIANT_TWO_SHOT
        # Propagate the resolved variant to a private config copy so the preamble
        # (which branches on config.all_reduce_variant) allocates the right
        # workspace, without mutating the caller's reusable config object.
    ...
    # If the caller did not supply a workspace, reuse a process-level cached one
    # keyed by (shmem, variant, shape, dtype) so repeated calls don't reallocate
    # symmetric-heap scratch on every invocation.
    cache_key = None
    if workspace is None:
        cache_key = (id(shmem), variant, M, N, input_tensor.dtype)
        workspace = _AR_WORKSPACE_CACHE.get(cache_key)
    ...
    needs_prepare = (
        ...
        or (variant == VARIANT_PUSH and (workspace.scratch is None))
        or (variant == VARIANT_TWO_SHOT_PUSH and (workspace.scratch is None))
    )
    ...
    if needs_prepare:
        ...
        if cache_key is not None:
            _AR_WORKSPACE_CACHE[cache_key] = workspace
    ...
    if variant == VARIANT_ATOMIC:
        ...
        persistent_all_reduce_two_shot[(config.comm_sms,)](
            ...
            num_warps=getattr(config, "all_reduce_num_warps", 8),
            num_stages=getattr(config, "all_reduce_num_stages", 1),
            waves_per_eu=getattr(config, "all_reduce_waves_per_eu", 1),
        )
    elif variant == VARIANT_TWO_SHOT_PUSH:
        if workspace is None or workspace.scratch is None:
            raise RuntimeError("two_shot_push requires workspace preparation. Call all_reduce_preamble first.")
        nw = getattr(config, "all_reduce_num_warps", 8)
        ns = getattr(config, "all_reduce_num_stages", 1)
        we = getattr(config, "all_reduce_waves_per_eu", 1)
        use_dev_barrier = getattr(config, "barrier_mode", "host") == "device"
        chunk_n = workspace.scratch.shape[1]

        # Process N in column chunks so each chunk's scratch stays L2-resident and
        # the reduce read-back avoids HBM at large N. chunk_n == N -> single shot.
        for n0 in range(0, N, chunk_n):
            in_v = input_tensor[:, n0 : n0 + chunk_n]
            out_v = output_tensor[:, n0 : n0 + chunk_n]
            # Phase 1: reduce-scatter scatter (remote WRITES into owners' scratch).
            ...
            # All scatters for this chunk must land before any owner reduces.
            if use_dev_barrier:
                shmem.device_barrier(group=group)
            else:
                shmem.barrier()
            # Phase 2: local reduce of owned block + all-gather broadcast (WRITES).
            persistent_all_reduce_ts_push_gather[(config.comm_sms,)](
                workspace.scratch,
                out_v,
                in_v,
                M,
            ...
            # Each chunk's gather writes the final block to peers; a trailing
            # barrier (below, for async_op) guarantees all chunks complete. An
            # extra per-chunk barrier is only needed before the NEXT chunk reuses
            # scratch — enforce it so scatter(k+1) can't clobber gather(k) reads.
            if n0 + chunk_n < N:
                if use_dev_barrier:
                    shmem.device_barrier(group=group)
                else:
        ...
        if workspace is None or workspace.scratch is None:
            raise RuntimeError("Push variant requires workspace preparation. Call all_reduce_preamble first.")

        # Phase 1: scatter local input into every peer's scratch (remote WRITE).
        persistent_all_reduce_push_scatter[(config.comm_sms,)](
            input_tensor,
            workspace.scratch,
            M,
            N,
        ...
        # All scatters must land on every rank before any rank reduces.
        shmem.barrier()
        # Phase 2: reduce the world_size local slots into output (local only).
        persistent_all_reduce_push_reduce[(config.comm_sms,)](
            workspace.scratch,
            output_tensor,
            M,
            N,
    ...
    if not async_op:
        # Cross-rank completion barrier: every output tile is written by its
        # owner rank via remote store, so a rank's output is only complete once
        # all peers have finished. "device" mode uses an on-GPU stream-ordered
        # atomic barrier (no ~370us host round-trip); "host" preserves the
        # legacy torch.cuda.synchronize() + distributed.barrier() path.
