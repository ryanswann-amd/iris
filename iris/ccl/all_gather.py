...
def persistent_all_gather(
    ...
    for tile_id in range(pid, total_tiles, COMM_SMS):
        ...
        # Send local shard data to all destination ranks. Stagger the destination
        # order per rank (i = (group_rank + j) % world_size) so concurrently active
        # WGs target DIFFERENT peers instead of hammering one xGMI link in lockstep.
...
def persistent_all_gather_pull(
    ...
    """
    Pull (GET) based all-gather kernel — direction-flipped twin of
    ``persistent_all_gather`` (which is push/PUT based).

    Each rank assembles its **own** full output buffer by *reading* every
    peer's input shard via ``iris.load`` (a remote READ that crosses xGMI as a
    load) and storing it into the correct local output row-block. This is the
    GET counterpart to the push kernel, which instead *writes* its single local
    shard out to every peer (a remote WRITE).

    The bytes that cross xGMI are identical to the push variant — ``(W-1)``
    shards per rank — but they travel as remote **reads** here instead of
    remote **writes**. This isolates the load-return-path ("LFIFO") cost the
    user's hypothesis is about. Note the local-store traffic differs (pull
    assembles the whole output locally), which is an inherent property of the
    direction, documented in the benchmark report.
    """
    pid = tl.program_id(0)
    ...
    for tile_id in range(pid, total_tiles, COMM_SMS):
        ...
        # Read every source rank's shard and place it into local output[i*M:].
        # Stagger the source order per rank to avoid all ranks hammering the
        # same peer's HBM at once (traffic shaping, mirrors the push kernel).
        for j in tl.static_range(world_size):
            i = (group_rank + j) % world_size
            source_rank = rank_start + i * rank_stride

            rm_output = rm_input + i * M
            output_mask = (rm_output[:, None] < (i + 1) * M) & (rn[None, :] < N)
            combined_mask = input_mask & output_mask
            ...
            if i == group_rank:
                # Local shard: plain local load + local store.
                data = tl.load(input_ptr_source, mask=combined_mask, other=0.0)
                tl.store(output_ptr_target, data, mask=combined_mask, cache_modifier=".wt")
            else:
                # Remote shard: GET (remote load that crosses xGMI as a read).
                # ".cg" streams through L2 without polluting L1 (project lesson).
                data = iris.load(
                    input_ptr_source,
                    iris_rank,
                    source_rank,
