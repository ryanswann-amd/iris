...
def persistent_reduce_scatter_two_shot(
    ...
    for tile_offset in range(pid, max_tile_offset, COMM_SMS):
        ...
        if is_full:
            ...
            acc = iris.load(
                base_ptr, iris_rank, start_rank_global, heap_bases, hint=(1, BLOCK_SIZE_N), cache_modifier=".cg"
            ).to(acc_dtype)
            for i in tl.static_range(1, world_size):
                remote_rank_idx = (start_rank_idx + i) % world_size
                remote_rank = rank_start + remote_rank_idx * rank_stride
                acc += iris.load(
                    base_ptr, iris_rank, remote_rank, heap_bases, hint=(1, BLOCK_SIZE_N), cache_modifier=".cg"
                ).to(acc_dtype)
