...
class IrisBase:
    ...
    def _apply_memory_format(
        ...
        if memory_format == torch.contiguous_format:
            ...
            tensor = self._create_tensor_with_strides(tensor, size, (C * H * W, 1, C * W, C))
            return tensor
            ...
            tensor = self._create_tensor_with_strides(tensor, size, (C * D * H * W, 1, C * D * W, C * W, C))
            return tensor
                ...
                if len(size) == 4:
                    ...
                    if len(input_strides) == 4 and input_strides[1] == 1:
                        ...
                        if len(input_shape) == 4:
                            ...
                            tensor = self._create_tensor_with_strides(tensor, new_size, input_strides)
                            return tensor
                elif len(size) == 5:
    ...
    def _create_tensor_with_strides(self, original_tensor: torch.Tensor, size: tuple, strides: tuple) -> torch.Tensor:
        """
    ...
    def empty(
        ...
        *size,
        ...
        tensor = self._apply_memory_format(tensor, size, memory_format)
