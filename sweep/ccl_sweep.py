...
def load_completed_keys(csv_path: str) -> set[str]:
    ...
    with open(csv_path, "r") as f:
        ...
        for row in reader:
            key = (
                f"{row['op']}|{row['m']}|{row['n']}|{row['num_gpus']}|"
...
def run_benchmark_point(point: SweepPoint, iris_root: str, heap_size: int = 1 << 34) -> Optional[dict]:
    ...
    cmd = [
        ...
        temp_output,
        "--block_size_m",
        str(point.block_size_m),
        "--block_size_n",
        str(point.block_size_n),
        "--swizzle_size",
        str(point.swizzle_size),
    ]
    ...
    try:
        ...
        if result.returncode != 0:
            print(
                f"  FAIL ({point.op} M={point.m} N={point.n} "
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Print parameter grid without executing")
    parser.add_argument("--resume", action="store_true", help="Resume from existing checkpoint CSV")
    parser.add_argument("--subset", type=float, default=1.0, help="Fraction of grid to run (0.0-1.0)")
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument("--heap-size", type=int, default=1 << 34, help="Iris heap size in bytes")
    ...
    for i, point in enumerate(pending):
        ...
        if data is not None:
            ...
            csvfile.flush()
            completed += 1
            print(f"  OK: {row['latency_ms']}ms, {row['bandwidth_gbps']} GB/s", flush=True)
        else:
