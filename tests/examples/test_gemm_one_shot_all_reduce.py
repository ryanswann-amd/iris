...
def test_gemm_one_shot_all_reduce_import():
    ...
    assert hasattr(benchmark_module, "gemm_one_shot_all_reduce"), "Benchmark module should have a gemm_one_shot_all_reduce function"
...
def test_block_size_calculations():
    ...
    assert actual_blocks_M == expected_blocks_M, f"Block M calculation mismatch: {actual_blocks_M} != {expected_blocks_M}"
    assert actual_blocks_N == expected_blocks_N, f"Block N calculation mismatch: {actual_blocks_N} != {expected_blocks_N}"
    assert actual_blocks_K == expected_blocks_K, f"Block K calculation mismatch: {actual_blocks_K} != {expected_blocks_K}"
...
def test_algorithm_parameters_validation():
    ...
    with pytest.raises(ValueError, match="Invalid number of stream-K SMs"):
        ...
        args_dict.update({
            "m": 64, "n": 64, "k": 64,
            "BLK_M": 32, "BLK_N": 32, "BLK_K": 16,
            "gsize_m": 1, "two_tiles": True, "num_stages": 1,
            "num_warps": 4, "waves_per_eu": 0, "mfmaInstrSize": 16, "kpack": 1,
        })
