...
"""
Benchmark for iris.ops matmul_all_reduce fused operation.

This benchmark showcases the fused GEMM + All-Reduce operation and reports
achieved TFLOPS and communication bandwidth.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Benchmark matmul_all_reduce fused operation.",
    ...
    parser.add_argument(
        "--output_file",
        type=str,
        default="matmul_all_reduce.json",
        help="Output file",
    )
    ...
    parser.add_argument(
        "--init_url", type=str, default="tcp://127.0.0.1:29528", help="Initialization URL for distributed setup"
    )
...
def _worker(local_rank: int, world_size: int, init_url: str, args: dict):
    ...
    config_kwargs = {
        ...
        "all_reduce_variant": args["all_reduce_variant"],
    }
    ...
    json_writer.add_field("operation", "matmul_all_reduce")
    ...
    # Must use shmem.zeros() to allocate on Iris symmetric heap
    ...
    # Reference: each rank computes local C = A @ B, then all_reduce
    if args["validate"]:
        expected_tensor = shmem.zeros((M, N), dtype=datatype)
        C_local_ref = torch.matmul(A_local_data, B_data)
        pytorch_output = C_local_ref.clone()
        shmem.barrier()
        dist.all_reduce(pytorch_output, op=dist.ReduceOp.SUM)
        torch.cuda.synchronize()
        expected_tensor.copy_(pytorch_output)
    ...
    def run_experiment():
        ...
        if hasattr(shmem.ops, "matmul_all_reduce_preamble"):
        ...
        torch.cuda.nvtx.range_push("Matmul-All-Reduce")
        with torch.cuda.stream(comm_stream):
            kernel_timing["matmul_all_reduce"]["start_event"].record()
            shmem.ops.matmul_all_reduce(
                C,
                A,
                B,
                config=config,
            ...
            kernel_timing["matmul_all_reduce"]["end_event"].record()
            kernel_timing["matmul_all_reduce"]["experiments"] += 1
        ...
        # Update timing
        ms = kernel_timing["matmul_all_reduce"]["start_event"].elapsed_time(
            kernel_timing["matmul_all_reduce"]["end_event"]
        )
        kernel_timing["matmul_all_reduce"]["ms"] += ms
    ...
    if args["validate"]:
        ...
        atol = 0.2 if datatype == torch.float16 else 0.3
        ...
        if success:
            shmem.info("Matmul-all-reduce validation passed!")
        else:
            shmem.error("Matmul-all-reduce validation failed!")
    ...
    if args["benchmark"]:
        ...
        tflops = total_tflops_unit / (
            (kernel_timing["matmul_all_reduce"]["ms"] / kernel_timing["matmul_all_reduce"]["experiments"]) * 1e-3
        )

        # Calculate bandwidth for all-reduce part
        ...
        shmem.info(
            f"Matmul-all-reduce (M={M}, N={N}, K={K}, world_size={world_size}, dtype={args['datatype']}, variant={args['all_reduce_variant']}): "
        ...
        json_writer.add_field(
            "matmul_all_reduce_ms",
            kernel_timing["matmul_all_reduce"]["ms"] / kernel_timing["matmul_all_reduce"]["experiments"],
        )
