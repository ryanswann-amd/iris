...
def parse_args():
    ...
    parser.add_argument("--output_file", type=str, default="ag_gemm_pipelined_push_log.json", help="Base name for output files")
...
def main():
    ...
    for config in configs_to_run:
        ...
        kernel_timing = {
            "push_kernel": { "start_event": torch.cuda.Event(enable_timing=True), "end_event": torch.cuda.Event(enable_timing=True), "ms": 0, "experiments": 0 },
            "wait_and_compute_kernel": { "start_event": torch.cuda.Event(enable_timing=True), "end_event": torch.cuda.Event(enable_timing=True), "ms": 0, "experiments": 0 }
        }
        ...
        def run_experiment():
            ...
            with torch.cuda.stream(main_stream):
                ...
                push_shards_kernel[push_grid](
                    ...
                    run_args["BLK_M"], run_args["BLK_K"],
                    rank, world_size, shmem.get_heap_bases()
                )
                ...
                wait_and_compute_gemm_kernel[(num_sms,)](
                    A_inbox_iris, B, C, M, N, K, signal_flags_iris,
                    A_inbox_iris.stride(0), A_inbox_iris.stride(1), A_inbox_iris.stride(2),
                    B.stride(0), B.stride(1),
                    C.stride(0), C.stride(1),
                    signal_flags_iris.stride(0), signal_flags_iris.stride(1), signal_flags_iris.stride(2), signal_flags_iris.stride(3),
                    run_args["BLK_M"], run_args["BLK_N"], run_args["BLK_K"], run_args["gsize_m"],
                    num_sms, 1, (K_local % run_args["BLK_K"] == 0),
                    rank, world_size
                )
                kernel_timing["wait_and_compute_kernel"]["end_event"].record()
            
            torch.cuda.synchronize()
            kernel_timing["push_kernel"]["ms"] += kernel_timing["push_kernel"]["start_event"].elapsed_time(kernel_timing["push_kernel"]["end_event"])
            kernel_timing["push_kernel"]["experiments"] += 1
            kernel_timing["wait_and_compute_kernel"]["ms"] += kernel_timing["wait_and_compute_kernel"]["start_event"].elapsed_time(kernel_timing["wait_and_compute_kernel"]["end_event"])
