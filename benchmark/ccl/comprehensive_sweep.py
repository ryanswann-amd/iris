...
def _make_rccl_runner(
    ...
    Buffers are torch tensors on the local CUDA device (no symmetric heap).
    """
    rank = dist.get_rank()
    device = torch.device(f"cuda:{torch.cuda.current_device()}")

    if collective == "all_reduce":
        tensor = torch.full((m, n), float(rank + 1), dtype=dtype, device=device)
        ...
        in_t = torch.full((m, n), float(rank + 1), dtype=dtype, device=device)
        out_t = torch.zeros((world_size * m, n), dtype=dtype, device=device)
        # Pre-build the per-rank slice list so the timed call only does the comm.
        ...
        in_t = torch.full((m, n), float(rank + 1), dtype=dtype, device=device)
        out_t = torch.zeros((m // world_size, n), dtype=dtype, device=device)
        if m % world_size:
            raise ValueError(
        ...
        in_t = torch.full((m, n), float(rank + 1), dtype=dtype, device=device)
        out_t = torch.zeros((m, n), dtype=dtype, device=device)
        per = n // world_size
        if per * world_size != n:
        ...
        for impl in args.impls:
            try:
                if impl == "iris":
                    ...
                    run, preamble = _make_rccl_runner(collective, m, n, world_size, dtype)
                else:
