...
def _max_bytes_for_collective(collective: str, world_size: int, max_per_rank_bytes: int) -> int:
    """Cap the per-rank input bytes for a collective.

    For all-gather the output is ``world_size`` × the input — sweeping the full
    1 GiB per rank would require an 8 GiB receive buffer per rank, which both
    iris and RCCL refuse on a 192 GiB MI300X once you account for staging
    buffers and the two-buffer (iris + RCCL pool) layout. The 1 GiB request is
    interpreted as "the largest payload size the kernel actually processes" —
    for all-gather that's the *output*, so we cap the per-rank input at
    ``max_per_rank_bytes / world_size`` instead.
    """
    if collective == "all_gather":
        return max_per_rank_bytes // world_size
    return max_per_rank_bytes
...
def _validation_grid(
    ...
    for total_bytes, dtype in itertools.product(sizes, dtypes):
        for collective in collectives:
            cap = _max_bytes_for_collective(collective, world_size, max_per_rank_bytes=1 << 30)
            if total_bytes > cap:
                continue
...
def _candidate_configs(collective: str, m: int, n: int, world_size: int) -> list[Config]:
    ...
    comm_sms_choices = [32, 64, 128, 256]
    block_n_choices = [32, 64, 128, 256]
    block_m_choices = [8, 32]
