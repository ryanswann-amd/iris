...
- **[22_gemm_one_shot_reduce_scatter_wg_specialization](https://github.com/ROCm/iris/tree/main/examples/22_gemm_one_shot_reduce_scatter_wg_specialization)**: GEMM + ReduceScatter with workgroup specialization, splitting SMs into GEMM and communication groups
- **[23_gemm_all_scatter_tracing](https://github.com/ROCm/iris/tree/main/examples/23_gemm_all_scatter_tracing)**: GEMM with all-scatter using DeviceContext tracing capabilities for performance analysis
...
- **[25_ccl_all_gather](https://github.com/ROCm/iris/tree/main/examples/25_ccl_all_gather)**: All-gather collective operation using `iris.ccl.all_gather` - concatenate tensors from all ranks
- **[26_ccl_all_to_all](https://github.com/ROCm/iris/tree/main/examples/26_ccl_all_to_all)**: All-to-all collective operation using `iris.ccl.all_to_all` - exchange data slices between all ranks

### Fused Operations (iris.ops)
- **[28_ops_matmul_all_reduce](https://github.com/ROCm/iris/tree/main/examples/28_ops_matmul_all_reduce)**: Fused GEMM + all-reduce using `iris.ops.matmul_all_reduce` - output = all_reduce(A @ B)
- **[29_ops_all_gather_matmul](https://github.com/ROCm/iris/tree/main/examples/29_ops_all_gather_matmul)**: Fused all-gather + GEMM using `iris.ops.all_gather_matmul` - output = all_gather(A_sharded) @ B
- **[30_ops_matmul_all_gather](https://github.com/ROCm/iris/tree/main/examples/30_ops_matmul_all_gather)**: Fused GEMM + all-gather using `iris.ops.matmul_all_gather` - output = all_gather(A_local @ B)

### Advanced Examples
- **[31_expert_sharded_moe](https://github.com/ROCm/iris/tree/main/examples/31_expert_sharded_moe)**: Expert-sharded Mixture of Experts (MoE) implementation with expert-parallel distributed inference
