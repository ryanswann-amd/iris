...
def push_shards_kernel(
    ...
    stride_sf_d, stride_sf_s, stride_sf_m, stride_sf_k,
    ...
    for tile_id in range(pid, total_tiles, NUM_SMS):
        ...
        for source_rank_id in range(world_size):
            ...
            for k_tile_idx in range(loop_k_tiles):
                flag_ptr = (signal_flags + cur_rank * stride_sf_d + source_rank_id * stride_sf_s + pid_m * stride_sf_m + k_tile_idx * stride_sf_k)
                while tl.load(flag_ptr, cache_modifier=".ca") == 0: pass

                k_offset = k_tile_idx * BLOCK_SIZE_K
                rk_local = k_offset + tl.arange(0, BLOCK_SIZE_K)
                A_ptr = (A_inbox + source_rank_id * stride_ai_rank + rm[:, None] * stride_ai_m + rk_local[None, :] * stride_ai_k)
...
def test_correctness():
    ...
    for b_size in batch_sizes:
        ...
        push_shards_kernel[push_grid](
            ...
            signal_flags_iris.stride(0), signal_flags_iris.stride(1), signal_flags_iris.stride(2), signal_flags_iris.stride(3),
            BLOCK_SIZE_M=BLOCK_M_GEMM, BLOCK_SIZE_K=BLOCK_K_GEMM,
        ...
        wait_and_compute_gemm_kernel[(num_sms,)](
            ...
            signal_flags_iris.stride(0), signal_flags_iris.stride(1), signal_flags_iris.stride(2), signal_flags_iris.stride(3),
            BLOCK_SIZE_M=BLOCK_M_GEMM, BLOCK_SIZE_N=64, BLOCK_SIZE_K=BLOCK_K_GEMM, GROUP_SIZE_M=6, NUM_SMS=num_sms,
            NUM_XCDS=1, EVEN_K=(K_local % BLOCK_K_GEMM == 0),
    ...
    for b_size in batch_sizes:
        ...
        def iris_push_gemm():
            ...
            wait_and_compute_gemm_kernel[(num_sms,)](
                ...
                signal_flags_iris.stride(0), signal_flags_iris.stride(1), signal_flags_iris.stride(2), signal_flags_iris.stride(3),
                BLOCK_SIZE_M=BLOCK_M_GEMM, BLOCK_SIZE_N=BLOCK_N_GEMM, BLOCK_SIZE_K=BLOCK_K_GEMM, GROUP_SIZE_M=6, NUM_SMS=num_sms,
                NUM_XCDS=1, EVEN_K=(K_local % BLOCK_K_GEMM == 0),
                cur_rank=rank, world_size=world_size
            )
        ...
        def rccl_ag_triton_gemm_full():
            ...
            local_gemm_kernel[(num_sms,)](
                ...
                BLOCK_SIZE_M=256, BLOCK_SIZE_N=64, BLOCK_SIZE_K=64, GROUP_SIZE_M=6, NUM_SMS=num_sms, NUM_XCDS=1, EVEN_K=(K % 64 == 0)
            )
