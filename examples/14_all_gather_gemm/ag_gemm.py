...
def test_correctness():
    ...
    for b_size in batch_sizes:
        ...
        persistent_ag_gemm[(num_sms,)](
            A_local_iris, B_replicated, C_local,
            M, N, K,
            ...
            GROUP_SIZE_M=8, NUM_SMS=num_sms, EVEN_K=True,
...
def test_performance():
    ...
    for b_size in batch_sizes:
        ...
        fn_to_benchmark_iris = lambda: persistent_ag_gemm[(num_sms,)](
            A_local_iris, B_sharded_src, C_local_iris, M, N, K,
            A_local_iris.stride(0), A_local_iris.stride(1), B_sharded_src.stride(0), B_sharded_src.stride(1),
            C_local_iris.stride(0), C_local_iris.stride(1),
            BLOCK_SIZE_M=256, BLOCK_SIZE_N=64, BLOCK_SIZE_K=64, GROUP_SIZE_M=6, NUM_SMS=num_sms, EVEN_K=True,
            heap_bases=iris_instance.get_heap_bases(), cur_rank=rank, world_size=world_size
        )
        time_iris_ms = iris.do_bench(fn=fn_to_benchmark_iris, barrier_fn=iris_instance.barrier, n_warmup=100, n_repeat=500, return_mode="mean")
        if rank == 0: print(f"  1. Iris Fused AG+GEMM Time: {time_iris_ms:.4f} ms")
        ...
        def rccl_ag_triton_gemm_full():
            ...
            local_gemm_kernel[(num_sms,)](
                ...
                A_gathered.stride(0), A_gathered.stride(1),
                B_sharded_src.stride(0), B_sharded_src.stride(1),
                C_local_triton.stride(0), C_local_triton.stride(1),
                BLOCK_SIZE_M=256, BLOCK_SIZE_N=64, BLOCK_SIZE_K=64,
                GROUP_SIZE_M=6, NUM_SMS=num_sms, EVEN_K=True
            )
    ...
    if rank == 0:
        ...
        headers = ["Batch Size", "Iris Fused (ms)", "RCCL+Triton (ms)", "RCCL+Torch (ms)", "Speedup (vs RCCL+Triton)", "Speedup (vs RCCL+Torch)"]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test', type=str, default='performance', choices=['correctness', 'performance'], help='Which test to run')
