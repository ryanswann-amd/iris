...
def normalize_blocks(x, BLOCK_SIZE=None):
    ...
    if x_ndim == 2:
        x = x.unsqueeze(0)
    for e, i, j in itertools.product(
        range(x.shape[0]), range(0, x.shape[1], BLOCK_SIZE), range(0, x.shape[2], BLOCK_SIZE)
    ):
...
def make_random_tensor(
    ...
    value_hbm_swizzling=None,
    value_hbm_swizzling_args={},
    ...
    if ragged_padding:
        buffer, ragged_metadata = pad_ragged_tensor(
            buffer, ragged_metadata, value_hbm_swizzling is not None or scale_hbm_swizzling is not None, ragged_dim == 1
        )
    # handle transpose
    if transpose:
