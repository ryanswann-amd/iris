...
@triton.jit()
def persistent_gemm_all_scatter_poolside(
    A,
    B,
    comm_data,
    Comm_M,
    ...
    COMM_SIZE_M: tl.constexpr,
    COMM_SIZE_N: tl.constexpr,
    ...
    if pid < GEMM_SMS:
        ...
        for tile_id in range(pid, total_tiles, GEMM_SMS):
            ...
            c = acc.to(c_global.type.element_ty)
        ...
        for tile_id in range(pid, comm_total_tiles, COMM_SMS):
            num_pid_in_group = GROUP_SIZE_M * comm_num_pid_n
            group_id = tile_id // num_pid_in_group
            first_pid_m = group_id * GROUP_SIZE_M
            group_size_m = min(comm_num_pid_m - first_pid_m, GROUP_SIZE_M)
            pid_m = first_pid_m + ((tile_id % num_pid_in_group) % group_size_m)
            pid_n = (tile_id % num_pid_in_group) // group_size_m

            # Begin: See the if segment for explanation:
            rm = (pid_m * COMM_SIZE_M + tl.arange(0, COMM_SIZE_M)) % Comm_M
            rn = (pid_n * COMM_SIZE_N + tl.arange(0, COMM_SIZE_N)) % Comm_N
            rm = tl.max_contiguous(tl.multiple_of(rm, COMM_SIZE_M), COMM_SIZE_M)
            rn = tl.max_contiguous(tl.multiple_of(rn, COMM_SIZE_N), COMM_SIZE_N)
            sub_mask = (rm[:, None] < Comm_N) & (rn[None, :] < Comm_N)
        ...
        if COLLECT_TIMESTAMPS:
            timestamp = read_realtime()
            tl.atomic_max(mm_end_timestamp_ptr + pid + GEMM_SMS, timestamp)
