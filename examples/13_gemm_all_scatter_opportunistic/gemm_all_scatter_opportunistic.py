...
from examples.common.utils import read_realtime, chiplet_swizzle
...
@triton.jit()
def persistent_gemm_all_scatter_opportunistic(
    A,
    B,
    C,
    c_global,
    ...
    pid = tl.program_id(0)
    pid = chiplet_swizzle(pid, NUM_SMS, NUM_XCDS)
    ...
    # kernel.
    if pid < GEMM_SMS:
        # First wave all workgroups perform GEMM.
        ...
        # For comms, we first have to start processing
        # tile 0 -> NUM_SMS, and then continue to process
        # remaining tiles until they are computed.
        pid = pid - GEMM_SMS
