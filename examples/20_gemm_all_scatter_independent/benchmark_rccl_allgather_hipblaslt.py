...
def benchmark(
    ...
    with torch.device("cuda"):
        ...
        # For all_gather, we need a list of output tensors
        gathered_tensors = [torch.empty_like(comm_tensor) for _ in range(world_size)]
    ...
    # warmup comm (all_gather)
    for _ in range(warmup_steps):
        with torch.cuda.stream(comm_stream):
            dist.all_gather(gathered_tensors, comm_tensor)
    torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if rank == 0:
        print(f"Using RCCL all_gather with world_size={world_size}")
    ...
    if rank == 0:
        prof.export_chrome_trace(f"rccl_allgather_trace_rank{rank}.json")
        print(f"Profiler trace saved to rccl_allgather_trace_rank{rank}.json")
