...
def _worker(local_rank: int, world_size: int, init_url: str, args: dict):
    ...
    def run_experiment():
        ...
        torch.cuda.nvtx.range_pop()
        shmem.barrier()
        comm_registers = comm_results.n_regs
        comm_spills = comm_results.n_spills
        json_writer.add_field("com_registers", comm_registers)
        json_writer.add_field("com_spills", comm_spills)
        if matmul._debug:
